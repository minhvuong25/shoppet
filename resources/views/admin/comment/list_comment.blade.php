                    ...
                    @foreach($comments as $key => $comment)
                        <tr>
                            ...
                            <td>{{$comment->comment}}
                                ...
                                <span class="noti_status text text-alert"></span>

{{--                                    <?php--}}
{{--                                    $check = \App\Models\Comment::where('comment_parent_comment', $comment->comment_id)->get();--}}
{{--                                if ($check->isEmpty()){--}}
{{--                                    ?>--}}
{{--                                <br><input id="reply_cmt" rows="1"--}}
{{--                                           class="reply_comment_{{$comment->comment_id}} form-control">--}}
{{--                                <br>--}}
{{--                                <button id="btn_reply_cmt" class="btn btn-default btn-xs btn-reply-comment"--}}
{{--                                        data-comment_id="{{$comment->comment_id}}"--}}
{{--                                        data-product_id="{{$comment->comment_product_id}}">Trả lời--}}
{{--                                </button>--}}
{{--                                    <?php--}}
{{--                                }--}}
{{--                                else {--}}
{{--                                    echo '<span class="text text-alert">Đã trả lời</span>';--}}
{{--                                }--}}
{{--                                    ?>--}}
                            </td>
                            <div id="notify_comment"></div>
                            <td>{{$comment->comment_date}}</td>
                            <td><a href="{{URL::to('chi-tiet-san-pham/'.$comment->product->product_slug)}}"
                                   target="_blank">{{$comment->product->product_name}}</a></td>
                            <td>
                                <a onclick="return confirm('Bạn có muốn xóa?')"
                                   href="{{URL::to('/delete-comment/'.$comment->comment_id)}}" class="active"
